...
// Creator
ModuleSceneBison::ModuleSceneBison (bool start_enabled):
Module (start_enabled)
{
  ...
  // Animation two mans
  two_mans_one_ground.frames.push_back (
					 {
					 {
					 144, 336, 24, 56}, 28,
					 {
  ...
  praying_man_ground.frames.push_back (
					{
					{
					177, 333, 30, 63}, 32,
					{
  ...
  praying_man_up.frames.push_back (
				    {
				    {
				    368, 329, 32, 62}, 31,
				    {
...
UpdateStatus ModuleSceneBison::Update ()
{
  ...
  rec_aux = { 32 + (int) (App->player_one->GetLife () * 89 / 200), 37,
    89 - (int) (App->player_one->GetLife () * 89 / 200), 8
  };
  App->renderer->DrawStaticRect (&rec_aux);
  rec_aux = { 137 + (int) (App->player_two->GetLife () * 89 / 200), 37,
    89 - (int) (App->player_two->GetLife () * 89 / 200), 8
  };
  App->renderer->DrawStaticRect (&rec_aux);
  ...
  if (restarting)
    {
      ...
      if (SDL_GetTicks () - timeRestarting > 5000
	  && SDL_GetTicks () - timeRestarting < 7000)
	{
	  SDL_SetRenderDrawColor (App->renderer->renderer, 0, 0, 0,
				  (SDL_GetTicks () - timeRestarting -
				   5000) / 8);
	}
      else if (SDL_GetTicks () - timeRestarting >= 7000
	       && SDL_GetTicks () - timeRestarting < 8000)
	{
	  SDL_SetRenderDrawColor (App->renderer->renderer, 0, 0, 0, 255);
	}
      else if (SDL_GetTicks () - timeRestarting >= 8000
	       && SDL_GetTicks () - timeRestarting < 10000
	       && actualizeFirstTime)
	{
	  SDL_SetRenderDrawColor (App->renderer->renderer, 0, 0, 0, 255);
	...
	{
	  SDL_SetRenderDrawColor (App->renderer->renderer, 0, 0, 0,
				  255 - (SDL_GetTicks () - timeRestarting -
					 8000) / 8);
	}
...
void
ModuleSceneBison::RestartScene (int wins)
{
