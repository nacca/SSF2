...
ModuleParticleSystem::ModuleParticleSystem (bool start_enabled):
...
UpdateStatus ModuleParticleSystem::Update ()
{
  for (list < Particle * >::iterator it = particleList.begin ();
      ...
      if (!(*it)->impact && (*it)->counter % 4 != 0)
	{
	  ...
	  (*it)->collider->rect.x =
	    (*it)->position.x +
	    (*it)->particleAnimation.GetCurrentCollider ().x;
	  (*it)->collider->rect.y =
	    (*it)->position.y +
	    (*it)->particleAnimation.GetCurrentCollider ().y;
	  (*it)->collider->rect.w =
	    (*it)->particleAnimation.GetCurrentCollider ().w;
	  (*it)->collider->rect.h =
	    (*it)->particleAnimation.GetCurrentCollider ().h;
	  if ((*it)->speed > 0)
	    {
...
bool
ModuleParticleSystem::newParticle (iPoint pos, SDL_Texture * graphics,
				   ...
				   int speed)
{
  ...
  Collider *collider =
    new Collider (rec_collider, ColliderType_Particles, this);
  Particle *particle =
    new Particle { pos, collider, graphics, particleAnimation,
    particleDestruction, speed, false, 0
  };
...
void
ModuleParticleSystem::OnCollision (Collider * c1, Collider * c2)
{
  for (list < Particle * >::iterator it1 = particleList.begin ();
...
int
ModuleParticleSystem::GetNumberParticles () const
{
...
int
ModuleParticleSystem::GetParticleSpeed (int id)
{
  list < Particle * >::iterator it1 = particleList.begin ();
  for (unsigned int i = 1; i < particleList.size (); ++i)
    ++it1;
