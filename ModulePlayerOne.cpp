...
update_status ModulePlayerOne::PreUpdate()
{
	...
	else if (hitted && playerState != PLAYER_CROUCH_HIT)
	{
...
// Update
update_status ModulePlayerOne::Update()
{
	...
	case PLAYER_WALKING_BACKWARD:
		if (looking_right)
		{
			...
			{
				if (!App->player_two->playerInCameraLimit())
				{
					position.x -= 1;
...
update_status ModulePlayerOne::PostUpdate()
{
