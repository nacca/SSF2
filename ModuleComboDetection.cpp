...
ModuleComboDetection::ModuleComboDetection (PlayerID playerID,
					    bool start_enabled):
...
void
ModuleComboDetection::UpdateCounters ()
{
...
void
ModuleComboDetection::UpdateComboState ()
{
...
void
ModuleComboDetection::UpdateComboStateNothing ()
{
  ...
  if (m_ModulePlayerDhalsim->GetPlayerInput (InputType_Down))
    {
      ...
      SetStartingComboParameters (PreviousComboState_Down,
				  ComboTypes_ComboYogaFire);
    }

  if (m_ModulePlayerDhalsim->GetPlayerInput (InputType_Down)
      && m_ModulePlayerDhalsim->IsJumping ()
      && m_ModulePlayerDhalsim->IsMovingBack ())
    {
      SetStartingComboParameters (PreviousComboState_DownBack,
				  ComboTypes_AerialCombo);
    }

  if (m_ModulePlayerDhalsim->IsMovingBack ())
    {
      SetStartingComboParameters (PreviousComboState_Back,
				  ComboTypes_ComboYogaFlame);
    }
}

void
ModuleComboDetection::UpdateComboStateBack ()
{
...
void
ModuleComboDetection::UpdateComboStateDownBack ()
{
  if (IsCountdownReached ())
    {
      RestartCombo ();
...
void
ModuleComboDetection::UpdateComboStateDown ()
{
  if (IsCountdownReached ())
    {
      RestartCombo ();
	   ...
	   && m_ModulePlayerDhalsim->IsMovingForward ())
    {
...
void
ModuleComboDetection::UpdateComboStateDownFront ()
{
  if (IsCountdownReached ())
    {
      RestartCombo ();
...
void
ModuleComboDetection::UpdateComboStateFront ()
{
  if (IsCountdownReached ())
    {
      RestartCombo ();
	...
	{
	  m_ModulePlayerDhalsim->SetStartingCombo
...
void
ModuleComboDetection::SetStartingComboParameters (PreviousComboState
						  previousState,
						  ComboTypes typeOfCombo)
{
...
void
ModuleComboDetection::StartFirstStateCountdown ()
{
  m_FirstStateCountdown = COMBO_STARTING_COUNTDOWNT;
}

void
ModuleComboDetection::StartPreviousStateCountdown ()
{
  m_PreviousStateCountdown = COMBO_NEW_COUNTDOWN;
}

void
ModuleComboDetection::RestartCombo ()
{
  m_IsAirCombo = false;
  m_PreviousState = PreviousComboState_Nothing;
  m_FirstStateCountdown = 0;
