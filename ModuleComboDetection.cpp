...
update_status ModuleComboDetection::PreUpdate()
{
	...
	switch (previous_state_player_one)
	{
		case NOTHING:
			...
			if ((App->input->GetKey(SDL_SCANCODE_DOWN) == KEY_REPEAT) &&
				(App->player_one->playerState == PLAYER_JUMPING) &&
				(App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_REPEAT && App->player_one->looking_right) ||
				(App->input->GetKey(SDL_SCANCODE_RIGHT) == KEY_REPEAT && !App->player_one->looking_right))
			{
			...
			if ((App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_REPEAT && App->player_one->looking_right) ||
				(App->input->GetKey(SDL_SCANCODE_RIGHT) == KEY_REPEAT && !App->player_one->looking_right))
			{
		...
		case COMBO_BACK:
			...
			else if (((App->input->GetKey(SDL_SCANCODE_RIGHT) == KEY_REPEAT) && !App->player_one->looking_right) ||
				((App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_REPEAT) && App->player_one->looking_right))
			{
				countdown_previous_state_player_one = 15;
			}
		...
		case COMBO_DOWN_FRONT:
			...
			{
				previous_state_player_one = NOTHING;
				App->player_one->starting_combo = AERIAL_COMBO_KICK;
				countdown_previous_state_player_one = 0;
				countdown_first_state_player_one = 0;
			}
			else if ((App->input->GetKey(SDL_SCANCODE_DOWN) == KEY_REPEAT) &&
		...
		case COMBO_FRONT:
			...
			if (countdown_previous_state_player_one <= 0 || countdown_first_state_player_one <= 0)
			{
				...
				(App->input->GetKey(SDL_SCANCODE_D) == KEY_DOWN))
			{
				if (App->player_one->playerState == PLAYER_JUMPING)
				{
					App->player_one->starting_combo = AERIAL_COMBO_PUNCH;
				}
				else if (type_of_combo_player_one == COMBO_YOGA_FIRE)
				{
					App->player_one->starting_combo = COMBO_YOGA_FIRE;
				}
				else if (type_of_combo_player_one == COMBO_YOGA_FLAME)
				{
					App->player_one->starting_combo = COMBO_YOGA_FLAME;
				}
				previous_state_player_one = NOTHING;
				countdown_previous_state_player_one = 0;
				countdown_first_state_player_one = 0;
	...
	case NOTHING:
		...
		if ((App->input->GetButton(SDL_CONTROLLER_AXIS_LEFTY) == KEY_REPEAT) &&
			(App->player_two->playerState == PLAYER_JUMPING) &&
			(App->input->GetKey(SDL_SCANCODE_LEFT) == KEY_REPEAT && App->player_two->looking_right) ||
			(App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTY) == KEY_REPEAT && !App->player_two->looking_right))
		{
		...
		if ((App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTX) == KEY_REPEAT && App->player_two->looking_right) ||
			(App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTY) == KEY_REPEAT && !App->player_two->looking_right))
		{
	...
	case COMBO_BACK:
		if (countdown_previous_state_player_two <= 0 || countdown_first_state_player_two <= 0)
		{
			...
			(((App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTY) == KEY_REPEAT) && !App->player_two->looking_right) ||
			((App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTX) == KEY_REPEAT) && App->player_two->looking_right)))
		{
			previous_state_player_two = COMBO_DOWN_BACK;
			countdown_previous_state_player_two = 15;
		}
		else if (((App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTY) == KEY_REPEAT) && !App->player_two->looking_right) ||
			((App->input->GetButton(SDL_CONTROLLER_AXIS_RIGHTX) == KEY_REPEAT) && App->player_two->looking_right))
		{
			countdown_previous_state_player_two = 15;
		}
	...
	case COMBO_DOWN_FRONT:
		...
		else if ((App->player_two->playerState == PLAYER_JUMPING) &&
			(App->input->GetButton(8) == KEY_DOWN) ||
			(App->input->GetButton(11) == KEY_DOWN) ||
			(App->input->GetButton(12) == KEY_DOWN))
		{
			previous_state_player_two = NOTHING;
			App->player_two->starting_combo = AERIAL_COMBO_PUNCH;
			countdown_previous_state_player_two = 0;
			countdown_first_state_player_two = 0;
		}
		else if ((App->player_two->playerState == PLAYER_JUMPING) &&
		...
		{
			previous_state_player_two = NOTHING;
			App->player_two->starting_combo = AERIAL_COMBO_KICK;
			countdown_previous_state_player_two = 0;
			countdown_first_state_player_two = 0;
		}
		else if ((App->input->GetButton(SDL_CONTROLLER_AXIS_LEFTY) == KEY_REPEAT) &&
	...
	case COMBO_FRONT:
		...
		if (countdown_previous_state_player_two <= 0 || countdown_first_state_player_two <= 0)
		{
			...
			(App->input->GetButton(12) == KEY_DOWN))
		{
			if (App->player_two->playerState == PLAYER_JUMPING)
			{
				App->player_two->starting_combo = AERIAL_COMBO_PUNCH;
			}
			else if (type_of_combo_player_two == COMBO_YOGA_FIRE)
			{
				App->player_two->starting_combo = COMBO_YOGA_FIRE;
			}
			else if (type_of_combo_player_two == COMBO_YOGA_FLAME)
			{
				App->player_two->starting_combo = COMBO_YOGA_FLAME;
			}
			previous_state_player_two = NOTHING;
			countdown_previous_state_player_two = 0;
			countdown_first_state_player_two = 0;
...
bool ModuleComboDetection::CleanUp()
{
	return UPDATE_CONTINUE;
}