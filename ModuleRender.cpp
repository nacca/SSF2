...
ModuleRender::Blit (SDL_Texture * texture, int x, int y, SDL_Rect * section,
		    float speed, SDL_RendererFlip flip_texture) const
{
  bool ret = true;
  SDL_Rect rect;
  ...
  if (SDL_RenderCopyEx
      (renderer, texture, section, &rect, 0.0f, NULL, flip_texture) != 0)
    {
...
// Draw a Rectangle
bool
ModuleRender::DrawRect (SDL_Rect * rec) const
{
  SDL_Rect rec_aux;
...
bool
ModuleRender::DrawStaticRect (SDL_Rect * rec) const
{
  SDL_Rect rec_aux;
...
bool
ModuleRender::ScreenLeftLimit () const
{
  if (camera.x == 0)
    return true;
  return false;
...
bool
ModuleRender::ScreenRightLimit () const
{
