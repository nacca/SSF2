...
ModulePlayerDhalsim::ModulePlayerDhalsim (PlayerID playerID,
					  bool start_enabled):
...
void
ModulePlayerDhalsim::SetUpAnimations ()
{
  Json::Value root;
  ...
  SetParticleAnimationDataFromJSON (m_DestroyParticle,
				    root["destroy-particle"][0]);
}

void
ModulePlayerDhalsim::SetUpPlayer ()
{
  if (m_PlayerID == PlayerID_One)
    {
      ...
      m_ColliderHead.type = m_ColliderBody.type = m_ColliderLegs.type =
  ...
  m_PlayerCollider.module = m_ColliderHead.module = m_ColliderBody.module =
    m_ColliderLegs.module = m_ColliderAttack.module = this;
}

// Load assets
bool
ModulePlayerDhalsim::Start ()
{
  LOG ("Loading Dhalsim");
...
void
ModulePlayerDhalsim::AddColliders ()
{
...
void
ModulePlayerDhalsim::DecideLookingPosition ()
{
...
ModulePlayerDhalsim::PreUpdate ()
{
  ...
  if (m_Dead)
    ...
    {
      if (m_PlayerState != PlayerState_Win1
  ...
  else if (m_PlayerState == PlayerState_BlockingHitted
  ...
  switch (m_PlayerState)
	       ...
	       GetPlayerInput (InputType_Up))
	{
      ...
      else if (GetPlayerInput (InputType_Right) && IsLookingRight ())
	{
	  m_PlayerState = PlayerState_WalkingForward;
	}
      else if (GetPlayerInput (InputType_Right) &&
	       ...
	       IsLookingRight () && m_OtherPlayer->IsAttacking ())
	{
      ...
      else if (GetPlayerInput (InputType_Left) && IsLookingRight ())
	{
	  m_PlayerState = PlayerState_WalkingBackward;
	}
      else if (GetPlayerInput (InputType_Left))
	       ...
	       && !m_JumpAttacked)
	{
      ...
      else if (m_StartingCombo == ComboTypes_AerialComboKick
	       && !m_JumpAttacked)
	{
	  m_AlreadyHitted = false;
	  m_JumpAttacked = true;
      ...
      else
	{
	  if (((GetPlayerInput (InputType_LPunch)) ||
	       ...
	       (GetPlayerInput (InputType_HPunch))) && !m_JumpAttacked)
	    {
	      m_JumpAttacked = true;
	      m_PlayerState = PlayerState_JumpPunch;
	    }
		    ...
		    (GetPlayerInput (InputType_HKick))) && !m_JumpAttacked)
	    {
	      m_JumpAttacked = true;
	      m_PlayerState = PlayerState_JumpKick;
	    }
    ...
    case PlayerState_YogaFire:
      if (yoga_fire.GetCurrentFrameNumber () == 3
	  && SDL_GetTicks () - m_LastShotTimer > 500)
	{
	  if (IsLookingRight ())
	    {
	      iPoint
	  ...
	  && (GetPlayerInput (InputType_Down)))
	{
	  ...
	  m_PlayerState = PlayerState_Crouching;
	       ...
	       && (!m_OtherPlayer->IsAttacking ()
		   || (IsLookingRight ()
		       && !(GetPlayerInput (InputType_Left)))
		   || (IsLookingLeft ()
		       && !(GetPlayerInput (InputType_Right)))))
	{
	  crouch_block.RestartFrames ();
	  m_PlayerState = PlayerState_Idle;
	}
...
ModulePlayerDhalsim::Update ()
{
  ...
  if (m_OtherPlayer->IsPlayerInCameraLimit () &&
	...
	&& m_OtherPlayer->GetLookingDirection () == LookingDirection_Right)
       || (App->renderer->ScreenRightLimit ()
	   && m_OtherPlayer->GetLookingDirection () == LookingDirection_Left))
  ...
  if (IsLookingRight ())
    {
      ...
      App->renderer->Blit (m_Graphics, m_Position.x - pivot,
			   m_Position.y - aux.h - m_DistanceJumped, &(aux),
			   1.0f, SDL_FLIP_NONE);
    }
  else
    {
      m_PlayerCollider.rect.x =
	m_Position.x - m_PlayerCollider.rect.x - m_PlayerCollider.rect.w;
      m_PlayerCollider.rect.y += m_Position.y - m_DistanceJumped;
      m_ColliderHead.rect.x =
	m_Position.x - m_ColliderHead.rect.x - m_ColliderHead.rect.w;
      m_ColliderHead.rect.y += m_Position.y - m_DistanceJumped;
      m_ColliderBody.rect.x =
	m_Position.x - m_ColliderBody.rect.x - m_ColliderBody.rect.w;
      m_ColliderBody.rect.y += m_Position.y - m_DistanceJumped;
      m_ColliderLegs.rect.x =
	m_Position.x - m_ColliderLegs.rect.x - m_ColliderLegs.rect.w;
      m_ColliderLegs.rect.y += m_Position.y - m_DistanceJumped;
      m_ColliderAttack.rect.x =
	m_Position.x - m_ColliderAttack.rect.x - m_ColliderAttack.rect.w;
      m_ColliderAttack.rect.y += m_Position.y - m_DistanceJumped;
      App->renderer->Blit (m_Graphics, m_Position.x - aux.w + pivot,
			   m_Position.y - aux.h - m_DistanceJumped, &(aux),
...
// On Collision
void
ModulePlayerDhalsim::OnCollision (Collider * c1, Collider * c2)
{
  if (!m_Win && !m_Dead && !m_Time0)
    {
      if (c1->type == ColliderType_BodyPlayerOne
	  && c2->type == ColliderType_BodyPlayerTwo)
	m_AreCollidingPlayers = true;
      else if (c1->type == ColliderType_BodyPlayerTwo
	       && c2->type == ColliderType_BodyPlayerOne)
	m_AreCollidingPlayers = true;
      else if (c1->type == ColliderType_PlayerOne
	       && c2->type == ColliderType_AttackPlayerTwo
	       && c2->rect.w * c2->rect.h != 0 && !m_AlreadyHitted)
	{
	  if (!m_Hitted && m_PlayerState != PlayerState_Blocking
	      && m_PlayerState != PlayerState_CrouchBlocking
	      && m_PlayerState != PlayerState_BlockingHitted
	      && m_PlayerState != PlayerState_CrouchBlockingHitted)
	    {
	    ...
	    if ((m_OtherPlayer->GetPlayerState () == PlayerState_CrouchPunch
		 || m_OtherPlayer->GetPlayerState () ==
      ...
      else if (c1->type == ColliderType_PlayerTwo
	       && c2->type == ColliderType_AttackPlayerOne
	       && c2->rect.w * c2->rect.h != 0 && !m_AlreadyHitted)
	{
	  if (!m_Hitted && m_PlayerState != PlayerState_Blocking
	      && m_PlayerState != PlayerState_CrouchBlocking
	...
	if ((c1->type == ColliderType_PlayerOne
	     || c1->type == ColliderType_AttackPlayerOne)
	    && c2->type == ColliderType_Particles
	    && c2->rect.w * c2->rect.h != 0)
	{
	  if (!m_Hitted && m_PlayerState != PlayerState_Blocking
	      && m_PlayerState != PlayerState_CrouchBlocking
	      && m_PlayerState != PlayerState_BlockingHitted
	...
	if ((c1->type == ColliderType_PlayerTwo
	     || c1->type == ColliderType_AttackPlayerTwo)
	    && c2->type == ColliderType_Particles
	    && c2->rect.w * c2->rect.h != 0)
	{
	  if (!m_Hitted && m_PlayerState != PlayerState_Blocking
...
bool
ModulePlayerDhalsim::IsPlayerInCameraLimit () const
{
  if (App->renderer->camera.x <=
      -((m_PlayerCollider.rect.x - m_PlayerCollider.rect.w) * SCREEN_SIZE))
    return true;
  if (App->renderer->camera.x - App->renderer->camera.w >=
      -((m_PlayerCollider.rect.x +
	 m_PlayerCollider.rect.w * 2) * SCREEN_SIZE))
    return true;
  return false;
}
...
void
ModulePlayerDhalsim::MovePlayer (int distance)
{
  if (distance > 0)
    {
	  ...
	  if (!m_OtherPlayer->IsPlayerInCameraLimit ())
	    {
	      m_Position.x += distance;
	      if (App->renderer->camera.x <=
		  -(m_PlayerCollider.rect.x * SCREEN_SIZE) +
		  (m_PlayerCollider.rect.w * SCREEN_SIZE))
		App->renderer->camera.x += SCREEN_SIZE;
	    }
	  else
	    {
...
void
ModulePlayerDhalsim::restartPlayer (bool everything)
{
...
bool
ModulePlayerDhalsim::IsAttacking () const
{
  if (m_PlayerState == PlayerState_LowPunch ||
      ...
      if (m_OtherPlayer->GetLookingDirection () == LookingDirection_Right)
	{
	  for (int i = 0; i < App->particles->GetNumberParticles (); ++i)
	    {
	      if (App->particles->GetParticlePosition (i).x >
		  m_OtherPlayer->m_Position.x
		  && App->particles->GetParticleSpeed (i) < 0)
		return true;
	    }
	}
      else
...
bool
ModulePlayerDhalsim::GetPlayerInput (InputType actionKey) const
{
  if (m_PlayerID == PlayerID_One)
    {
      switch (actionKey)
...
void
ModulePlayerDhalsim::SetSDLRectFromData (SDL_Rect & sdl_rect,
...
void
ModulePlayerDhalsim::SetPlayerAnimationDataFromJSON (Animation & animation,
  ...
  for (int i = 0; i < numberOfFrames; ++i)
    {
      ...
      SetSDLRectFromData (animationStructure.colliders.Position_collider,
			  currentValue["position-collider"]);
      SetSDLRectFromData (animationStructure.colliders.Collider_head,
			  currentValue["head-collider"]);
      SetSDLRectFromData (animationStructure.colliders.Collider_body,
			  currentValue["body-collider"]);
      SetSDLRectFromData (animationStructure.colliders.Collider_legs,
			  currentValue["legs-collider"]);
      SetSDLRectFromData (animationStructure.colliders.Collider_attack,
			  currentValue["attack-collider"]);
  ...
  for (int i = 0; i < numberOfFrames; ++i)
    {
      ...
      SetSDLRectFromData (particleAnimationStructure.frame,
			  currentValue["frame"]);

      particleAnimationStructure.pivot = currentValue["pivot"].asInt ();

      SetSDLRectFromData (particleAnimationStructure.collider,
			  currentValue["collider"]);
...
int
ModulePlayerDhalsim::GetWins () const
{
  return m_Wins;
}

void
ModulePlayerDhalsim::SetWins (int wins)
{
  m_Wins = wins;
}

int
ModulePlayerDhalsim::GetLife () const
{
  return m_Life;
}

void
ModulePlayerDhalsim::SetLife (int life)
{
  m_Life = life;
}

void
ModulePlayerDhalsim::DecreseLife (int life)
{
  m_Life -= life;
}

bool
ModulePlayerDhalsim::IsWin () const
{
  return m_Win;
}

void
ModulePlayerDhalsim::SetWin (bool win)
{
  m_Win = win;
}

bool
ModulePlayerDhalsim::IsDead () const
{
  return m_Dead;
}

void
ModulePlayerDhalsim::SetDead (bool dead)
{
  m_Dead = dead;
}

bool
ModulePlayerDhalsim::IsTime0 () const
{
  return m_Time0;
}

void
ModulePlayerDhalsim::SetTime0 (bool time_0)
{
  m_Time0 = time_0;
}

bool
ModulePlayerDhalsim::IsJumping () const
{
...
bool
ModulePlayerDhalsim::IsMovingBack () const
{
  bool isMovingBack = false;
  isMovingBack = isMovingBack || (GetPlayerInput (InputType_Left)
				  && IsLookingRight ());
  isMovingBack = isMovingBack || (GetPlayerInput (InputType_Right)
				  && IsLookingLeft ());
  return isMovingBack;
}

bool
ModulePlayerDhalsim::IsMovingForward () const
{
  bool isMovingForward = false;
  isMovingForward = isMovingForward || (GetPlayerInput (InputType_Right)
					&& IsLookingRight ());
  isMovingForward = isMovingForward || (GetPlayerInput (InputType_Left)
					&& IsLookingLeft ());
  return isMovingForward;
}

bool
ModulePlayerDhalsim::IsPunchInput () const
{
...
bool
ModulePlayerDhalsim::IsKickInput () const
{
...
bool
ModulePlayerDhalsim::IsLookingRight () const
{
  return m_LookingDirection == LookingDirection_Right;
}

bool
ModulePlayerDhalsim::IsLookingLeft () const
{
  return m_LookingDirection == LookingDirection_Left;
}

void
ModulePlayerDhalsim::SetLookingRight (LookingDirection lookingDirection)
{
...
void
ModulePlayerDhalsim::SetPlayerState (PlayerState playerState)
{
...
void
ModulePlayerDhalsim::SetStartingCombo (ComboTypes starting_combo)
{
  m_StartingCombo = starting_combo;
}

int
ModulePlayerDhalsim::GetDistanceJumped () const
{
