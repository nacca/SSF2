...
UpdateStatus ModuleCollisions::Update ()
{
  ...
  for (list < Collider * >::iterator it1 = colliders.begin ();
       it1 != colliders.end (); ++it1)
    {
      list < Collider * >::iterator it2 = it1;
      ++it2;
      while (it2 != colliders.end ())
	{
	  if (CanCollide ((*it1)->type, (*it2)->type)
	      && IsCollision ((*it1)->rect, (*it2)->rect))
	    {
...
UpdateStatus ModuleCollisions::PostUpdate ()
{
  if (show_colliders)
    {
      for (list < Collider * >::iterator it = colliders.begin ();
	   it != colliders.end (); ++it)
	{
	  if ((*it)->type == ColliderType_Particles
	      || (*it)->type == ColliderType_AttackPlayerOne
	      || (*it)->type == ColliderType_AttackPlayerTwo)
	    SDL_SetRenderDrawColor (App->renderer->renderer, 255, 0, 0, 100);
	  else if ((*it)->type == ColliderType_PlayerOne
		   || (*it)->type == ColliderType_PlayerTwo)
	    SDL_SetRenderDrawColor (App->renderer->renderer, 0, 0, 255, 100);
	  else if ((*it)->type == ColliderType_BodyPlayerOne
		   || (*it)->type == ColliderType_BodyPlayerTwo)
...
void
ModuleCollisions::AddCollider (SDL_Rect rec, ColliderType type,
			       Module * module)
{
...
void
ModuleCollisions::AddCollider (Collider * collider)
{
  colliders.push_back (collider);
}

bool
ModuleCollisions::IsCollision (SDL_Rect rec1, SDL_Rect rec2)
{
  if (!(rec2.x > rec1.x + rec1.w
	|| rec2.x + rec2.w < rec1.x
	|| rec2.y > rec1.y + rec1.h || rec2.y + rec2.h < rec1.y))
    return true;
  else
    return false;
}
...
bool
ModuleCollisions::CanCollide (ColliderType type1, ColliderType type2)
{
